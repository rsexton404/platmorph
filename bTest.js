...
bTest.prototype.getBodyAtMouse = function() {
...
bTest.prototype.getBodyCB = function(fixture) {
...
if (!connecting) {
  if(isMouseDown && (!mouseJoint) ) {
               ...
               last_clicked = body;
               if(body) {
					if (freebees <= 2 && (body.m_userData == "hyper" || body.m_userData == "ist") ) {
						  connections_made++;
						  freebees++;
						  updateStars();
					}
	              ...
	              body.SetAngle(0);
                  fixedY = body.GetPosition().y;
                  var md = new b2MouseJointDef();
                  md.bodyA = this.world.GetGroundBody();
                  md.bodyB = body;
                  md.target.Set(mouseX, fixedY);
            ...
            if(mouseJoint) {
               if(isMouseDown && fixedY ) {
                  mouseJoint.SetTarget(new b2Vec2(mouseX, fixedY));
               } else {
...
bTest.prototype.setBodies = function(bodyEntities) {
    ...
    for(var id in bodyEntities) {
        ...
        if (entity.character_type == 'platform' || entity.character_type == 'wall') {
            bodyDef.type = b2Body.b2_staticBody;
        } else {
